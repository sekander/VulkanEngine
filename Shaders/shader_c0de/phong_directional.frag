...
void main() {
    // int i = 1; // use index 1 for directional light
    ...
    for(int i = 0; i < 3; i++) {
        vec3 lightDir = normalize(-uboLightData.position[i]); // direction stored in position[i]
        ...
        rim = pow(rim, 2.0); // Sharpen rim effect
        vec3 rimLighting = push.rimLightStrength * rim * push.rimLightColour;

        totalLighting += ambient + diffuse + specular + rimLighting;
    }
